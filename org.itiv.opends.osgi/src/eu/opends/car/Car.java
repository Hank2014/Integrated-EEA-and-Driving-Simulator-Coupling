...
public abstract class Car {
	...
	protected Geometry hitBoxGeometry;
	protected Geometry rainGeometry;
	protected Geometry frontGeometryRadar;
	protected Geometry frontGeometryRadarVector;
	...
	protected float[] ODRelatedParameters = new float[] {0.0f, 0.0f, 0.0f};
...
/*	 protected void init()
	...
	private void setupTrailer() {
		...
		CarModelLoader trailerModelLoader = new CarModelLoader(sim, this, trailerModelPath, trailerMass, new Vector3f(), new Quaternion());
		...
		/*
		 * // apply joint HingeJoint joint=new
		 * HingeJoint(carNode.getControl(VehicleControl.class),
		 * trailerNode.getControl(VehicleControl.class), new Vector3f(0f, 0f,
		 * 3f), // pivot point local to carNode new Vector3f(0f, 0f, -1.5f), //
		 * pivot point local to trailerNode Vector3f.UNIT_Y, // DoF Axis of
		 * carNode (Y axis) Vector3f.UNIT_Y); // DoF Axis of trailerNode (Y
		 * axis) joint.setCollisionBetweenLinkedBodys(false);
		...
		// apply joint1
		HingeJoint joint1 = new HingeJoint(carNode.getControl(VehicleControl.class), connector,
		...
		// apply joint
		HingeJoint joint2 = new HingeJoint(connector, trailerNode.getControl(VehicleControl.class),
				new Vector3f(0f, 0f, 0f), // pivot point local to connector
	...
	private void setupReferencePoints() {
		...
		centerNode.setName("centerNode");
		centerNode.attachChild(centerGeometry);
		centerNode.setCullHint(CullHint.Always);
		getCarNode().attachChild(centerNode);

		// add node representing position of rain box
		Box rainBox = new Box(1f, 1f, 1f);
		rainGeometry = new Geometry("rainBox", rainBox);
		rainGeometry.setLocalTranslation(0, 0, -20f);
		Material rainMaterial = new Material(sim.getAssetManager(), "Common/MatDefs/Misc/Unshaded.j3md");
		rainMaterial.setColor("Color", ColorRGBA.Black);
		rainGeometry.setMaterial(rainMaterial);
		Node rainNode = new Node();
		rainNode.setName("rainNode");
		rainNode.attachChild(rainGeometry);
		rainNode.setCullHint(CullHint.Always);
		getCarNode().attachChild(rainNode);
		
		Box frontBoxRadar = new Box(0.01f, 0.01f, 0.01f);
		frontGeometryRadar = new Geometry("frontBoxRadar", frontBoxRadar);
		frontGeometryRadar.setLocalTranslation(0, 0.5f, 0);
		Material frontMaterialRadar = new Material(sim.getAssetManager(), "Common/MatDefs/Misc/Unshaded.j3md");
		frontMaterialRadar.setColor("Color", ColorRGBA.Blue);
		frontGeometryRadar.setMaterial(frontMaterialRadar);
		Node frontNodeRadar = new Node();
		frontNodeRadar.setName("frontNodeRadar");
		frontNodeRadar.attachChild(frontGeometryRadar);
		frontNodeRadar.setCullHint(CullHint.Always);
		getCarNode().attachChild(frontNodeRadar);
		
		Box frontBoxRadarVector = new Box(0.01f, 0.01f, 0.01f);
		frontGeometryRadarVector = new Geometry("frontBoxRadarVector", frontBoxRadarVector);
		frontGeometryRadarVector.setLocalTranslation(0, 0.5f, -2.2f);
		Material frontMaterialRadarVector = new Material(sim.getAssetManager(), "Common/MatDefs/Misc/Unshaded.j3md");
		frontMaterialRadarVector.setColor("Color", ColorRGBA.Red);
		frontGeometryRadarVector.setMaterial(frontMaterialRadarVector);
		Node frontNodeRadarVector = new Node();
		frontNodeRadarVector.setName("frontNodeRadarVector");
		frontNodeRadarVector.attachChild(frontGeometryRadarVector);
		frontNodeRadarVector.setCullHint(CullHint.Always);
		getCarNode().attachChild(frontNodeRadarVector);
	...
	/*public Vector3f getPositionCenterRadar(){
		for (int i=0; i<carNode.getChildren().size();i++){
			System.out.println("List of childrens = " + i + carNode.getChild(i).getName());
			System.out.println("Coordinates of childrens are = " + carNode.getChild(i).getWorldTransform());
		}
		return carNode.getChild("frontNodeRadar").getWorldTranslation();
	}
	
	public Vector3f getPositionCenterRadarVector(){
		return carNode.getChild("frontNodeRadarVector").getWorldTranslation();
	}*/
	...
	public float getSlope() {
		...
		// slope in radians (with correction due to different suspension
		// heights)
	...
	public void setEmergencyLights(boolean setToOn) {
		lightTexturesContainer.setEmergencyLight(setToOn);
	}
	...
	public void setODRelatedParameters(float LatOffFrontC, float YawRateFild, float speedLateral) {
		ODRelatedParameters[0] = LatOffFrontC;		
		ODRelatedParameters[1] = YawRateFild;
		ODRelatedParameters[2] = speedLateral;
	}
	
	public float[] getODRelatedParameters() {
		float[] dst = ODRelatedParameters;
		return dst;
	}
}
