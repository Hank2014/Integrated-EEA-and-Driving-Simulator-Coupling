...
public class CANClient extends Thread
{
	...
	public synchronized void sendCarData()
	{
		...
		if(forwardEvent(currentTime))
		{
			...
			Vector3f geoPosition = car.getGeoPosition().toVector3f();
			...
			float accPedalIntensity = car.getAcceleratorPedalIntensity();
			float acc = car.getAcceleration();
			
			/*if(car.getCurrentSpeedKmh()<30) {
			...
			try { 	
				...
				String positionString = "$SimCarState#Speed=" + speed + " AccelerationpedalIntensity"+car.getAcceleratorPedalIntensity()+" acc="+ car.getAcceleration() ;
    ...
    private static long timeDiff(Calendar timestamp1, Calendar timestamp2) throws NullPointerException
    {
        try {
