...
public class SkyPropertiesReader
{
	...
	{
		File file = new File(System.getProperty("user.dir")+"/assets/" + filename);
		if(!file.exists())
			throw new AssetNotFoundException(filename);
		
		FileInputStream inputStream = new FileInputStream(System.getProperty("user.dir")+"/assets/" + filename);
        ...
        Vector3f normalScale = new Vector3f(-1f, 1f, 1f);
        float SphereRadius = 50f;
        
		return SkyFactory.createSky(assetManager, west, east, north, south, up, down, normalScale, SphereRadius);
	}
