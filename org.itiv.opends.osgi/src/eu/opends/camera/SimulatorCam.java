	...
	public void setCamMode(CameraMode mode)
	{
		switch (mode)
		{
			...
			case STATIC_BACK:
				...
				frontCameraNode.setLocalRotation(new Quaternion().fromAngles(-0.12f, 0, 0));
				break;
...
/*		if(digitalMapViewCamNode != null)
		...
		//default: screenshot starts from 300+ frame, now: 0+
		if(frameCounter%5==0 && frameCounter <= 2000) //&& frameCounter >= 300 
		{
	...
	public byte[] takePeriodicRGB() throws IOException {
		screenshotAppState_rgb.takeScreenshot();
		byte[] img = null;
		img = screenshotAppState_rgb.postFrameForSocket(rgbViewPort, Image.Format.RGBA8, ScreenshotFormat.valueOf("PNG"), settingsLoader.getSetting(Setting.General_radarCamera_trimHeight, 0), "");
		return img; 
	}
	
	
	@SuppressWarnings("unsued")
	private void takeRGBScreenshot() {
		if(rgbViewPort != null)
		{
			...
			ScreenshotFormat outputFormat_rgb = ScreenshotFormat.valueOf("PNG");
			...
			//screenshotAppState_rgb.postFrame(rgbViewPort, Image.Format.RGBA8, outputFormat_rgb, trimHeight, value);
		}
	}
	...
	private void takeRadarScreenshot()
	{
		if(radarViewPort != null)
		{
			...
			//screenshotAppState.postFrame(radarViewPort,Format.Depth32F, ScreenshotFormat.PGM_BINARY, 100);
			//screenshotAppState.postFrame(radarViewPort,Format.Depth32F, ScreenshotFormat.FLOAT_MAP, 100);
			//screenshotAppState.postFrame(radarViewPort,Format.Depth32F, outputFormat, trimHeight, screenshotAppState_rgb.getFileRGBName());
			//screenshotAppState.postFrame(radarViewPort,Format.RGBA8, outputFormat_rgb, trimHeight);
		}
	}
...
/*	private void updateDigitalMap() 
	...
	public int getCamCount() {
		int cc = 0;
		if(screenshotAppState_rgb.getDoneWritingScreenshot()) {
			cc = this.camCount;
		}
		return cc;
	}
